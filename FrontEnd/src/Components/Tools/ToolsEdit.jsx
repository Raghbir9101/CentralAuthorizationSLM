...
function ToolsEdit({ selectedRow, setSelectedRow }) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            let { data } = await axios.put(`tools/${selectedRow._id}`, postData);
            setTools(prev => {
                let tempPrev = [...prev];
                for (let i = 0; i < tempPrev.length; i++) {
                    if (tempPrev[i]._id == selectedRow._id) {
                        tempPrev[i] = data;
                        break
                    }
                }
    ...
    return (
        ...
        >
            <Box component="form" onSubmit={handleSubmit} sx={style}>
                <TextField defaultValue={selectedRow?.toolName || ""} name='toolName' label="Tool Name" required />
                <TextField defaultValue={selectedRow?.toolID || ""} name='toolID' label="Tool ID" required />
                <TextField defaultValue={selectedRow?.toolLink || ""} name='toolLink' label="Tool Link" required />
